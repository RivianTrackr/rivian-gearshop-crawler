...
_OFFERS_EXPORT_SQL = """
SELECT
  o.id, o.slug, o.title, o.url, o.cta_url, o.expiration,
  o.body_hash, o.removed, o.first_seen_at, o.last_seen_at, o.updated_at
FROM offers o
ORDER BY o.title
"""


def _offers_export_rows(db_path: str) -> list[dict]:
    cdb = get_crawler_db(db_path)
    try:
        rows = cdb.execute(_OFFERS_EXPORT_SQL).fetchall()
        return [
            {
                "id": r["id"],
                "slug": r["slug"],
...
@router.get("/data/{script_id}/export-offers.json")
def export_offers_json(script_id: int):
    script = _get_script(script_id)
    if not script or not script["db_path"] or not os.path.exists(script["db_path"]):
        return RedirectResponse("/", status_code=303)

    rows = _offers_export_rows(script["db_path"])
    content = json.dumps({"count": len(rows), "offers": rows}, indent=2, ensure_ascii=False)
    return StreamingResponse(
        io.BytesIO(content.encode("utf-8")),
        media_type="application/json",
        headers={"Content-Disposition": f'attachment; filename="offers-export.json"'},
    )


@router.get("/data/{script_id}/export-offers.csv")
def export_offers_csv(script_id: int):
    script = _get_script(script_id)
    if not script or not script["db_path"] or not os.path.exists(script["db_path"]):
        return RedirectResponse("/", status_code=303)
    ...
    return StreamingResponse(
        io.BytesIO(output.getvalue().encode("utf-8")),
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="offers-export.csv"'},
    )


# ---------------------- Offers Routes ----------------------

def _is_offers_db(db_path: str) -> bool:
    """Check if the database contains offers tables."""
    cdb = get_crawler_db(db_path)
    try:
        tables = {r[0] for r in cdb.execute(
            "SELECT name FROM sqlite_master WHERE type='table'"
        ).fetchall()}
        return "offers" in tables
    except Exception:
        return False
    finally:
        cdb.close()
...
@router.get("/data/{script_id}/offers", response_class=HTMLResponse)
def offers_list(request: Request, script_id: int,
                page: int = Query(1, ge=1),
                q: str = Query("")):
    script = _get_script(script_id)
    if not script or not script["db_path"] or not os.path.exists(script["db_path"]):
    ...
    try:
        if q:
            count_row = cdb.execute(
                "SELECT COUNT(*) as cnt FROM offers WHERE title LIKE ? OR slug LIKE ?",
                (f"%{q}%", f"%{q}%"),
            ).fetchone()
        else:
            count_row = cdb.execute("SELECT COUNT(*) as cnt FROM offers").fetchone()
        ...
        if q:
            offers = cdb.execute(
                """SELECT o.*,
                          (SELECT COUNT(*) FROM offer_snapshots s WHERE s.offer_id = o.id) as snapshot_count
                   FROM offers o
                   WHERE o.title LIKE ? OR o.slug LIKE ?
...
@router.get("/data/{script_id}/offers/{offer_id}", response_class=HTMLResponse)
def offer_detail(request: Request, script_id: int, offer_id: int):
    script = _get_script(script_id)
    if not script or not script["db_path"] or not os.path.exists(script["db_path"]):
        return RedirectResponse("/", status_code=303)
    ...
    try:
        ...
        if not offer:
            return RedirectResponse(f"/data/{script_id}/offers", status_code=303)
        ...
        snapshots = cdb.execute(
            """SELECT * FROM offer_snapshots
               ...
               ORDER BY crawled_at DESC
               LIMIT ?""",
            (offer_id, SNAPSHOT_HISTORY_LIMIT),
        ).fetchall()
    finally:
        cdb.close()
...
@router.get("/data/{script_id}/offers-crawl-history", response_class=HTMLResponse)
def offers_crawl_history(request: Request, script_id: int, page: int = Query(1, ge=1)):
    script = _get_script(script_id)
    if not script or not script["db_path"] or not os.path.exists(script["db_path"]):
        return RedirectResponse("/", status_code=303)
    ...
    cdb = get_crawler_db(script["db_path"])
    try:
        count_row = cdb.execute("SELECT COUNT(*) as cnt FROM offers_crawl_stats").fetchone()
        total = count_row["cnt"]
        total_pages = max(1, math.ceil(total / PAGE_SIZE))
        offset = (page - 1) * PAGE_SIZE

        stats = cdb.execute(
            "SELECT run_at, offer_count AS item_count FROM offers_crawl_stats ORDER BY run_at DESC LIMIT ? OFFSET ?",
            (PAGE_SIZE, offset),
        ).fetchall()
    finally:
        cdb.close()
